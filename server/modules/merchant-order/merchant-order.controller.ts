...
export class MerchantOrderController {
  ...
  async getOrders(
    ...
    return this.merchantOrderService.getOrderList(
      req.user.id,
      pageNum,
      pageSizeNum,
      status,
  ...
  async getOrderDetail(
    ...
    return this.merchantOrderService.getOrderDetail(req.user.id, id);
  }
  ...
  async acceptOrder(
    ...
    return this.merchantOrderService.acceptOrder(req.user.id, id);
  }
  ...
  async rejectOrder(
    ...
    return this.merchantOrderService.rejectOrder(req.user.id, id, reason);
  }
  ...
  async progressOrder(
    @Req() req: Request & { user: JwtPayload },
    @Param('id') id: string,
    @Body() body: { targetStatus: OrderStatus },
  ) {
    const targetStatus = body?.targetStatus;
    if (!targetStatus) {
      throw new BadRequestException('targetStatus 不能为空');
    }
    return this.merchantOrderService.progressOrder(
      req.user.id,
      id,
      targetStatus,
    );
